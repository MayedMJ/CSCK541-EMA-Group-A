...
client_box_labels = {
    ...
    "Flight": ["Client ID", "Airline ID", "Date", "Start City", "End City"],
}
...
def create_button(text, row, column, size, store, action_key):
    ...
    btn = ctk.CTkButton(
        ...
        command=lambda: prepare_action_data(
            text, option_types[action_key].lower(), store
        ),
    )
...
message_label = ctk.CTkLabel(
    buttons_frame, text="", font=("Arial", 20), text_color="green"
)
    ...
    try:
        if text == "Create Record":
            ...
            textbox_info = service.search_records(
                record_type, **prepare_payload(store, text)
            )
                ...
                for key, value in record.items():
                    if key != "type":
                        store["Results"][1].insert(
                            "end", f"{variable_label_mapping[key]}: {value}\n"
                        )
        ...
        message_label.configure(
            text=f"Validation Error: {e}", text_color="red"
        )
    except RecordConflictError as e:
        message_label.configure(text=f"Conflict Error: {e}", text_color="red")
    except RecordNotFoundError as e:
        message_label.configure(text=f"Not Found Error: {e}", text_color="red")
...
def build_panel(frame, record_type, store):
    ...
    if frame in (create_options_frame, update_options_frame):
        labels = (
            ...
            if frame == create_options_frame and record_type != "Flight"
            else [
                label
                for label in client_box_labels[record_type]
                if label != "Type"
            ]
        )
    ...
    if frame == search_options_frame:
        for i, label in enumerate(
            ...
            if record_type != "Flight"
                ...
                label
                for label in client_box_labels[record_type]
                if label not in ["Type", "Client ID", "Airline ID"]
            ]
        ):
            lbl = ctk.CTkLabel(frame, text=label, font=("Arial", 12))
            ent = ctk.CTkEntry(frame, width=150)
        ...
        lbl.grid(
            row=len(client_box_labels[record_type]),
            column=0,
            padx=10,
            pady=5,
            sticky="w",
...
# UI creation helpers
def create_title():
    ctk.CTkLabel(upper_frame, text=title, font=("Arial", 30)).grid(
...
def update_create():
    ...
    if option_types["Search"] == "Client":
        ...
        option_types["Create"] == "Client"
        or option_types["Update"] == "Client"
        or option_types["Search"] == "Client"
    ):
...
def run_gui(service_instance=None):
    global service
